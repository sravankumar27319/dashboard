...
function LineTooltip({ active, payload, label }) {
  if (!active || !payload?.length) return null;
  return (
    <div style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 12, padding: "10px 14px", boxShadow: "0 8px 24px rgba(0,0,0,0.25)" }}>
...
function ChartCard({ title, subtitle, children, legend }) {
  return (
    <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-6 shadow-sm flex flex-col h-[350px] sm:h-[400px]">
      <div className="mb-3">
...
export default function Charts() {
  ...
  if (!data) return <div className="p-10 text-center">Loading Charts...</div>;
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Bar dataKey="income" radius={[4, 4, 0, 0]} onMouseEnter={(_, i) => setActiveIdx(i)}>
              {monthly.map((_, i) => (
                <Cell key={i} fill={INCOME_COLOR} opacity={activeIdx !== null && activeIdx !== i ? 0.3 : 1} />
              ))}
            </Bar>
            <Bar dataKey="expenses" radius={[4, 4, 0, 0]} onMouseEnter={(_, i) => setActiveIdx(i)}>
              {monthly.map((_, i) => (
                <Cell key={i} fill={EXPENSE_COLOR} opacity={activeIdx !== null && activeIdx !== i ? 0.3 : 1} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
